...
def view_customers():
    ...
    for customer in customers:
        customer["agent_level"] = _agent_level(customer.get("agent_level"))
...
def update_customer(customer_id):
    ...
    if "agent_level" in request.form:
        submitted_level = (request.form.get("agent_level") or "").strip().lower()
        if submitted_level not in AGENT_LEVELS:
            return jsonify({"status": "error", "message": "Invalid agent level"}), 400
        data["agent_level"] = submitted_level
    ...
    # Protect fields
    for key in ("_id", "customer_id", "role", "deleted", "deleted_at"):
        data.pop(key, None)

    now = datetime.utcnow()
    data["updated_at"] = now
    if "agent_level" in data:
        data["agent_level_updated_at"] = now

    res = users_col.update_one({"_id": oid, "role": "customer"}, {"$set": data})
    if res.matched_count:
        ensure_user_balance(oid, now)
    return jsonify({
