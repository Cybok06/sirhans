...
@complaints_bp.route("/complaints/<complaint_id>/store-status", methods=["POST"])
def update_store_complaint_status(complaint_id):
    user_id = session.get("user_id")
    if not user_id:
        flash("You must be logged in to manage complaints.", "danger")
        return redirect(url_for("login.login"))

    complaint_oid = _try_objectid(complaint_id)
    user_oid = _try_objectid(user_id)
    new_status = (request.form.get("status") or "").strip().lower()
    allowed = {"pending", "in_progress", "resolved", "rejected"}
    if not complaint_oid or not user_oid or new_status not in allowed:
        flash("Invalid complaint update.", "warning")
    ...
    flash("Complaint status updated." if result.modified_count else "Complaint not found.", "success" if result.modified_count else "warning")
    return redirect(url_for("complaints.view_complaints"))


@complaints_bp.route("/complaints/<complaint_id>/forward-admin", methods=["POST"])
def forward_store_complaint_to_admin(complaint_id):
    user_id = session.get("user_id")
    if not user_id:
        flash("You must be logged in to forward complaints.", "danger")
        return redirect(url_for("login.login"))

    complaint_oid = _try_objectid(complaint_id)
    user_oid = _try_objectid(user_id)
    if not complaint_oid or not user_oid:
        flash("Invalid complaint.", "warning")
        return redirect(url_for("complaints.view_complaints"))
    ...
    if result.modified_count:
        flash("Complaint forwarded to the admin team successfully.", "success")
    else:
        existing = complaints_col.find_one({"_id": complaint_oid, "user_id": user_oid}, {"forwarded_to_admin": 1})
        flash("This complaint has already been forwarded." if existing else "Complaint not found.", "info" if existing else "warning")
    return redirect(url_for("complaints.view_complaints"))
