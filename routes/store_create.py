...
_LOCAL_WHATSAPP_RE = re.compile(r"^0\d{9}$")
...
def _validate_store_whatsapp(value: str) -> Tuple[bool, str]:
    number = (value or "").strip()
    if number and not _LOCAL_WHATSAPP_RE.fullmatch(number):
        return False, "WhatsApp number must use the format 0XXXXXXXXX."
    return True, number
...
def _enforce_mtn_exclusive_selection(payload: Dict[str, Any]) -> Tuple[bool, str]:
    # MTN NORMAL and MTN EXPRESS are allowed to coexist in a store.
    # Keep this hook as a no-op for backward compatibility with callers.
    return True, ""
...
def api_upsert_store():
    ...
    whatsapp_ok, whatsapp_number = _validate_store_whatsapp(whatsapp_number)
    if not whatsapp_ok:
        return jsonify({"success": False, "message": whatsapp_number}), 400
