...
SENDER_ID = "Sir Hans"  # requested sender name
...
def _user_snapshot(u: dict) -> dict:
    return {
        ...
        "phone": u.get("phone", ""),
        "phone_normalized": u.get("phone_normalized", ""),
    }
...
def _build_user_search_query(q: str) -> dict:
    ...
    if q:
        ...
        conditions.extend(token_conditions)

    return {"$and": conditions}


def _matches_user_search(user: dict, q: str) -> bool:
    if not q:
        return True
    ...
    phone = (user.get("phone") or "").strip().lower()
    phone_normalized = (user.get("phone_normalized") or "").strip().lower()

    if q_norm in full_name or q_norm in username or q_norm in phone or q_norm in phone_normalized:
        return True

    if q_digits and (q_digits in re.sub(r"\D+", "", phone) or q_digits in re.sub(r"\D+", "", phone_normalized)):
        return True
...
def view_balances():
    """
    List balances with server-side user search.
    Optional query params:
      q=  (filters by full name, username, or phone)
      limit= (default 200)
    """
    ...
    user_query = _build_user_search_query(q)
    users_raw = list(
        users_col.find(
            user_query,
            {"first_name": 1, "last_name": 1, "username": 1, "phone": 1, "phone_normalized": 1},
        )
        .sort([("_id", -1)])
        .limit(limit)
    )

    user_ids = [u["_id"] for u in users_raw if u.get("_id")]
    balances_map = {}
    if user_ids:
        for bal in balances_col.find(
            {"user_id": {"$in": user_ids}},
            {"user_id": 1, "amount": 1, "currency": 1, "updated_at": 1},
        ):
            balances_map[bal["user_id"]] = bal

    balances = []
    for raw_user in users_raw:
        user = _user_snapshot(raw_user)
        if not _matches_user_search(user, q):
            continue

        bal = balances_map.get(user["_id"])
        if not bal:
            bal = ensure_user_balance(user["_id"])
            if not bal:
                continue
    ...
    return render_template("admin_balance.html", balances=balances, q=q)
