...
from db import db
from admin_access import can_manage_providers
...
_ALLOWED_PROVIDERS = {"portal02", "dataconnect", "codecraft", "datakazina", "skplug", "bundleportal"}
...
def create_service():
    if not _require_admin():
        return redirect(url_for("login.login"))
    if "provider" in request.form and not can_manage_providers():
        return jsonify({"success": False, "error": "Only full administrators can select providers."}), 403
...
def set_service_provider(service_id):
    if not _require_admin():
        return jsonify({"success": False, "error": "Unauthorized"}), 401
    if not can_manage_providers():
        return jsonify({"success": False, "error": "Only full administrators can change providers."}), 403
    try:
        _id = ObjectId(service_id)
    except Exception:
        return jsonify({"success": False, "error": "Invalid service id"}), 400
    ...
    if not provider_val:
        return jsonify({"success": False, "error": "provider must be 'portal02', 'dataconnect', 'codecraft', 'datakazina', 'skplug', or 'bundleportal'"}), 400
