...
def inject_admin_counts():
    ...
    """
    customer_store_reports_count = 0
    if not _is_admin():
        try:
            user_id = ObjectId(str(session.get("user_id") or ""))
            customer_store_reports_count = complaints_col.count_documents(
                {
                    "user_id": user_id,
        ...
        return {
            ...
            "customer_store_reports_count": customer_store_reports_count,
        }
    ...
    try:
        pending_complaints = complaints_col.count_documents(
            {
                "status": "pending",
                "$or": [
                    {"complaint_type": {"$ne": "store_report"}},
