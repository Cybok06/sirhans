...
EXPORTABLE_ORDER_STATUSES = {"pending", "processing"}
...
def _collect_undelivered_rows(args):
    ...
    for order in orders_col.find(query).sort([("created_at", 1)]):
        if _normalize_status_text(order.get("status")) not in EXPORTABLE_ORDER_STATUSES:
            continue
...
def _collect_selected_undelivered_rows(args):
    ...
    now = datetime.utcnow()
    if not rows:
        raise ValueError("Only pending and processing orders can be exported.")
    timeframe = {
        "mode": "selected",
...
def admin_view_orders():
    ...
    try:
        ...
        for o in orders:
            ...
            valid_providers = {"codecraft", "dataconnect", "portal02", "datakazina", "skplug"}
