...
CODECRAFT_API_KEY = os.getenv("CODECRAFT_API_KEY", "260129025618-iafWYf-|FJJLo-ov1b8V-0?vzDK-AYNMWV")
...
# ===== CodeCraft package cache + submit helpers ==============================
def _codecraft_get_packages_cached():
    ...
    headers = {"Accept": "application/json", "x-api-key": CODECRAFT_API_KEY}
    ...
    try:
        ...
        if resp.status_code not in (200, 201):
            jlog("codecraft_packages_http_error", status=resp.status_code, body_len=len(text))
            return {}
        ...
        package_map = {}
        for row in (data.get("regular_packages") or []) + (data.get("bigtime_packages") or []):
            if not isinstance(row, dict):
                continue
            network = str(row.get("network") or "").upper()
            try:
                gig = int(float(row.get("package")))
                amount = float(row.get("amount"))
            except Exception:
                continue
            if network:
                package_map.setdefault((network, gig), amount)

        _CODECRAFT_CACHE["ts"] = now
        _CODECRAFT_CACHE["packages"] = package_map
        ...
        jlog("codecraft_packages_parse_error", error=str(e))
        return {}


def _codecraft_submit(phone, gig, network, external_ref, order_id, debug_events):
    if not CODECRAFT_API_KEY:
        err = {"success": False, "error": "CODECRAFT API key not configured", "type": "CONFIG_ERROR", "http_status": 500}
        jlog("codecraft_config_error", order_id=order_id, ref=external_ref, reason="missing_api_key")
    ...
    masked = phone[:5] + "***" + phone[-2:] if phone and len(phone) >= 7 else "***"
    jlog(
        "codecraft_request_body",
        order_id=order_id,
        ref=external_ref,
        body={"recipient_number": masked, "gig": body["gig"], "network": body["network"]},
    )
    ...
    try:
        ...
        ref = payload.get("reference_id") if isinstance(payload, dict) else None
        status_val = payload.get("status") if isinstance(payload, dict) else None
        msg = str(payload.get("message") or "").lower() if isinstance(payload, dict) else ""
        ok = bool(ref) and (status_val == 200 or payload.get("success") is True or "recorded" in msg)
        if isinstance(payload, dict):
            payload.setdefault("http_status", resp.status_code)

        jlog("codecraft_response", order_id=order_id, ref=external_ref, payload=payload)
        jlog("codecraft_call", order_id=order_id, ref=external_ref, ok=ok, status=resp.status_code)
        debug_events.append(
            {
                "when": datetime.utcnow(),
                "stage": "codecraft-initiate",
                "ok": ok,
                "http_status": resp.status_code,
            }
        )
        return ok, payload, ref

    except requests.RequestException as e:
        jlog("codecraft_network_error", order_id=order_id, ref=external_ref, error=str(e))
...
def process_checkout():
    try:
        ...
        for idx, item in enumerate(cart, start=1):
            ...
            if svc_provider_from_db not in ("portal02", "dataconnect", "codecraft", "datakazina", "skplug"):
                jlog(
            ...
            jlog(
                ...
                selected_provider=(
                    "codecraft" if use_codecraft else "datakazina" if use_datakazina else "dataconnect" if use_dataconnect else "skplug" if use_skplug else "portal02" if use_portal02 else "manual"
                ),
            )
            ...
            if use_codecraft:
                ...
                if not phone or not network or not gig:
                    has_processing = True
                    total_processing_amount += amt_total
                    results.append(
                        {
                            ...
                            "api_response": {
                                "note": "CodeCraft fields missing; queued for processing",
                                "got": {"phone": bool(phone), "network": network, "gig": gig},
                            },
                            "provider": "codecraft",
                        }
                    )
                ...
                if (network, gig) in package_map:
                    provider_amount = package_map[(network, gig)]
                    ...
                    results.append(
                        {
                            ...
                            "api_response": {
                                "note": "CodeCraft package not found for network/gig; queued for manual processing",
                                "network": network,
                                "gig": gig,
                            },
                            "provider": "codecraft",
                ...
                line_record = {
                    ...
                    "line_status": "processing",
                    "api_status": "queued",
                    "api_response": {"note": "Queued for background CodeCraft call"},
                }
            ...
            if use_datakazina:
                ...
                line_record = {
                    ...
                    "line_status": "processing",
                    "api_status": "queued",
                    "api_response": {"note": "Queued for background DataKazina call"},
                }
            ...
            if use_dataconnect:
                ...
                line_record = {
                    ...
                    "line_status": "processing",
                    "api_status": "queued",
                    "api_response": {"note": "Queued for background DataConnect call"},
                }
            ...
            if use_skplug:
                ...
                line_record = {
                    ...
                    "line_status": "processing",
                    "api_status": "queued",
                    "api_response": {"note": "Queued for background SKPlug call"},
                }
            ...
            try:
                for split_order_id, split_jobs in order_jobs:
                    t = threading.Thread(
                        ...
                        args=(split_order_id, split_jobs),
                        daemon=True,
                    )
                    t.start()
            except Exception as e:
                jlog("checkout_bg_spawn_error", order_id=primary_order_id, error=str(e))
