...
from flask import Blueprint, current_app, render_template, session, redirect, url_for, request, flash, jsonify, send_file
...
ALLOWED_STATUSES   = {"pending", "processing", "delivered", "failed", "completed", "refunded", "cancelled"}
ALLOWED_SORTS      = {"newest", "oldest", "amount_desc", "amount_asc"}
ALLOWED_ORDER_SCOPES = {"all", "single", "bulk"}
DEFAULT_PER_PAGE   = 10
...
def _normalize_order_scope(value):
    scope = (value or "all").strip().lower()
    return scope if scope in ALLOWED_ORDER_SCOPES else "all"


def _apply_order_scope_to_query(query, scope):
    scoped = dict(query or {})
    if scope == "single":
        scoped["items.0"] = {"$exists": True}
        scoped["items.1"] = {"$exists": False}
    elif scope == "bulk":
        scoped["items.1"] = {"$exists": True}
    return scoped


def _classify_order(items):
    count = len(items or [])
    return ("bulk", count) if count > 1 else ("single", count)
...
def _order_refund_base(order):
    """Historical base total, capped by the amount actually charged."""
    def amount(value):
        try:
            result = Decimal(str(value))
        except (InvalidOperation, ValueError, TypeError):
            raise ValueError("Invalid recorded base price or charge.")
...
def _normalize_status_text(value):
    # Some older orders have non-string provider/status values.  Exports should
    # not fail just because one line has one of those values.
...
def _normalize_provider(value):
    ...
    if key == "skplug" or ("sk" in key and "plug" in key):
        return "skplug"
    if "codecraft" in key:
        return "codecraft"
    if key in {"bundleportal", "bundleportalapi"}:
        return "bundleportal"
    if "dataconnect" in key or "dconnect" in key:
        return "dataconnect"
    if key in {"portal02", "portal2", "portal002"} or ("portal" in key and "02" in key):
        return "portal02"
    if key in {"datakazina", "dakazina"} or ("kazina" in key and ("data" in key or "daka" in key)):
        return "datakazina"
    return raw
...
def _collect_undelivered_rows(args):
    ...
    for order in orders_col.find(query).sort([("created_at", 1)]):
        ...
        for idx, item in enumerate(items):
            if not isinstance(item, dict) or not _is_undelivered_line(item):
                continue
...
def _collect_selected_undelivered_rows(args):
    raw_ids = (args.get("order_ids") or "").split(",")
    order_ids = []
    for raw_id in raw_ids:
        raw_id = raw_id.strip()
        if raw_id and ObjectId.is_valid(raw_id):
            order_ids.append(ObjectId(raw_id))

    if not order_ids:
        raise ValueError("Select at least one order to export.")
    ...
    for order in orders_col.find({"_id": {"$in": order_ids}}).sort([("created_at", 1)]):
        if isinstance(order.get("created_at"), datetime):
            created_dates.append(order["created_at"])
        for idx, item in enumerate(order.get("items") or []):
            if not isinstance(item, dict):
                continue
            # Explicitly selected exports also support retrying failed lines.
            status = _normalize_status_text(item.get("line_status") or item.get("status"))
            if status != "failed" and not _is_undelivered_line(item):
                continue
            service_name = _normalize_text(item.get("serviceName"))
            ...
            offer_text = _display_offer_from_item(item)
            rows.append({
                "source": _normalize_source(order.get("source")),
                "order_id": order.get("order_id") or str(order.get("_id")),
                "line_id": _line_identifier(order, item, idx),
                "line_index": idx,
                "service": service_name,
                "network": _resolve_item_network(item, service_network_map),
                "phone": _normalize_text(item.get("phone")),
                "offer": offer_text,
                "compact_offer": _compact_export_offer(offer_text),
                "created_at": order.get("created_at"),
    ...
    now = datetime.utcnow()
    if not rows:
        raise ValueError("The selected orders do not contain any exportable lines.")
    timeframe = {
        "mode": "selected",
        "label": "Selected orders",
        "date_from": min(created_dates) if created_dates else now,
        "date_to": max(created_dates) if created_dates else now,
    }
    return rows, timeframe, service_names
...
def _group_export_rows_by_network(rows, selected_network=None):
    groups = {}
    fallback_network = _export_header_text(selected_network)
    for row in rows:
        network = _normalize_network_name(row.get("network")) or fallback_network
        groups.setdefault(network, []).append(row)
    return groups

def _build_export_text_lines(rows, selected_network):
    lines = []
    for network, network_rows in _group_export_rows_by_network(rows, selected_network).items():
        if lines:
            lines.append("")
...
def _render_excel_export(rows, selected_network):
    ...
    for network, network_rows in _group_export_rows_by_network(rows, selected_network).items():
        if sheet_rows:
            sheet_rows.append(["", ""])
        sheet_rows.append([network, ""])
        for row in network_rows:
            sheet_rows.append([
                row.get("phone") or "",
                row.get("compact_offer") or "",
            ])
...
def _render_pdf_export(rows, selected_network):
    ...
    story = []
    for group_index, (network, network_rows) in enumerate(
        _group_export_rows_by_network(rows, selected_network).items()
    ):
        if group_index:
            story.append(Spacer(1, 18))
        story.append(Paragraph(network, styles["Title"]))
        story.append(Spacer(1, 12))
        for row in network_rows:
            phone = row.get("phone") or ""
            compact_offer = row.get("compact_offer") or ""
            story.append(Paragraph(f"{phone}&nbsp;&nbsp;&nbsp;&nbsp;{compact_offer}", styles["Normal"]))
    ...
    for oid in order_ids:
        try:
            ...
            if isinstance(order.get("items"), list) and order.get("items"):
                update_doc["items.$[].line_status"] = new_status
            ...
            if new_status == "refunded":
                ...
                if charged_amount > 0 and user_id and not already_refunded:
                    ...
                    try:
                        transactions_col.insert_one({
                            ...
                            "verified_at": now,
                            "meta": {
                                "note": f"{reason.capitalize()} base price refund",
                                "refund_basis": "base_price",
                                "order_db_id": oid,
                ...
                update_doc["refunded_at"] = order.get("refunded_at") or now
...
def admin_view_orders():
    ...
    order_scope = _normalize_order_scope(request.args.get("order_scope"))
    base_query = _build_query_from_params(request.args)
    query = _apply_order_scope_to_query(base_query, order_scope)
    ...
    try:
        order_scope_counts = {
            "all": orders_col.count_documents(base_query),
            "single": orders_col.count_documents(_apply_order_scope_to_query(base_query, "single")),
            "bulk": orders_col.count_documents(_apply_order_scope_to_query(base_query, "bulk")),
        }
        ...
        page_order_ids = {str(order.get("_id")) for order in orders}
        exported_by_order = {}
        if page_order_ids:
            batch_cursor = export_batches_col.find(
                {"lines.order_db_id": {"$in": list(page_order_ids)}},
                {"badge_no": 1, "created_at": 1, "lines.order_db_id": 1},
            ).sort([("created_at", -1)])
            for batch in batch_cursor:
                matched_order_ids = {
                    str(line.get("order_db_id"))
                    for line in (batch.get("lines") or [])
                    if str(line.get("order_db_id")) in page_order_ids
                }
                for order_db_id in matched_order_ids:
                    info = exported_by_order.setdefault(order_db_id, {
                        "count": 0,
                        "latest_badge_no": batch.get("badge_no") or "",
                        "latest_created_at": batch.get("created_at"),
                    })
                    info["count"] += 1

        for o in orders:
            try:
                o["refund_base_amount"] = _order_refund_base(o)
            except ValueError:
            ...
            valid_providers = {"codecraft", "dataconnect", "portal02", "datakazina", "skplug", "bundleportal"}
            ...
            for item in (o.get("items") or []):
                ...
                if prov in valid_providers and api_status and api_status not in NOT_PASSED_STATUSES:
                    ...
                    if api_status in PASSED_STATUSES:
                        ...
                        api_passed = True
                elif prov == "datakazina" and not api_status:
        ...
        order_scope_counts = {"all": 0, "single": 0, "bulk": 0}
    ...
    return render_template(
        ...
        order_scope_counts=order_scope_counts,
...
def update_order_status(order_id):
    ...
    if updated:
        msg = {
            ...
            "cancelled": "Order marked as Cancelled. Any previous refund is preserved.",
...
def bulk_deliver_orders():
    ...
    try:
        ...
        modified, errors = _apply_status_change(
            ids,
            "delivered",
            reason="bulk",
...
def bulk_deliver_selected():
    ...
    try:
        ...
        flash(f"✅ Marked {modified} selected order(s) as Delivered.", "success")
        if errors:
            flash(" | ".join(errors[:3]), "warning")
    except Exception:
...
def export_undelivered_orders():
    ...
    selected_mode = bool((request.form.get("order_ids") or "").strip())
    if fmt not in {"txt", "excel", "pdf"} or (selected_mode and fmt not in {"excel", "pdf"}):
        flash("Invalid export format.", "danger")
        return redirect(url_for("admin_orders.admin_view_orders"))

    try:
        if selected_mode:
            rows, timeframe, service_names = _collect_selected_undelivered_rows(request.form)
        else:
        ...
        current_app.logger.exception("Unable to prepare undelivered order export")
    ...
    network = request.form.get("network") or "ANY"
    source_filter = "selected" if selected_mode else (request.form.get("source") or "all").strip().lower()
