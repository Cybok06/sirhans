...
def _split_codecraft_package_maps() -> Tuple[Dict[Tuple[str, int], float], Dict[Tuple[str, int], float]]:
    """
    Accept either the legacy `(regular_map, bigtime_map)` shape or the current
    merged package map returned by checkout.py.
    """
    raw = _codecraft_get_packages_cached()
    if isinstance(raw, tuple) and len(raw) == 2:
        regular_map = raw[0] if isinstance(raw[0], dict) else {}
        bigtime_map = raw[1] if isinstance(raw[1], dict) else {}
...
def store_public_page(slug: str):
    ...
    return render_template(
        "store_page.html",
        announcements=get_active_announcements("store_page"),
        store=store_doc,
        services=priced,
        products=products,
...
def place_store_order(payload: Dict[str, Any], channel: str = "store_web") -> Tuple[Dict[str, Any], int]:
    ...
    for idx, item in enumerate(cart, start=1):
        ...
        if svc_provider not in ("portal02", "dataconnect", "codecraft", "datakazina", "skplug"):
        ...
        if api_allowed and svc_provider == "datakazina":
            ...
            if phone and shared_bundle:
                api_status = "queued"
                api_note = "Queued for background DataKazina call."
                network_id = 3
                job_payload = {
                    "provider_request_order_id": external_ref,
                    "phone": phone,
            ...
            if phone and shared_bundle and dc_network_id:
                api_status = "queued"
                api_note = "Queued for background DataConnect call."
        ...
        elif api_allowed and svc_provider == "codecraft":
            network = _resolve_codecraft_network(svc_doc, item)
            gig = _resolve_codecraft_gig(value_obj, item)
            regular_map, bigtime_map = _split_codecraft_package_maps()
            mode = None
            provider_amount = None
            if network == "MTN":
                if (network, gig) in regular_map:
                    mode = "regular"
                    provider_amount = regular_map[(network, gig)]
                elif (network, gig) in bigtime_map:
                    mode = "regular"
                    provider_amount = bigtime_map[(network, gig)]
            else:
                if (network, gig) in bigtime_map:
                    mode = "bigtime"
                    provider_amount = bigtime_map[(network, gig)]
                elif (network, gig) in regular_map:
                    mode = "regular"
                    provider_amount = regular_map[(network, gig)]
            provider_name = "codecraft"
            provider_network = network
            if phone and network and gig and mode:
                api_status = "queued"
                api_note = "Queued for background CodeCraft call."
                job_payload = {
                    "provider_request_order_id": external_ref,
                    "phone": phone,
                    "provider": "codecraft",
                ...
                api_status = "skipped_missing_fields"
                api_note = "CodeCraft fields missing or package not found; queued for manual processing."
            ...
            if phone and skplug_network and package_size_gb is not None:
                api_status = "queued"
                api_note = "Queued for background SKPlug call."
                job_payload = {
                    "provider_request_order_id": external_ref,
                    "phone": phone,
                    "provider": "skplug",
            ...
            provider_network = resolved_network
            api_status = "queued"
            api_note = "Queued for background Portal-02 call."
            job_payload = {
                "provider_request_order_id": external_ref,
                "phone": phone,
                "provider": "portal02",
    ...
    if order_jobs and not defer_provider_processing:
        try:
            ...
            jlog("store_checkout_bg_spawn_error", order_id=primary_order_id, error=str(e))
