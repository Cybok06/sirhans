...
def _split_codecraft_package_maps() -> Tuple[Dict[Tuple[str, int], float], Dict[Tuple[str, int], float]]:
    """
    Backward-compatible adapter for CodeCraft package cache helpers.
    Older callers expected `(regular_map, bigtime_map)`, while the current
    checkout helper returns a single merged package map.
    """
...
def _create_public_order_from_verified_payment(
    ...
    for idx, item in enumerate(server_cart, start=1):
        ...
        use_codecraft = (svc_provider == "codecraft") and api_allowed
        use_datakazina = (
        ...
        jlog(
            ...
            selected_provider=(
                "codecraft" if use_codecraft else "datakazina" if use_datakazina else "dataconnect" if use_dataconnect else "skplug" if use_skplug else "portal02" if portal02_network_slug else "manual"
            ),
        )
                ...
                results.append(
                    {
                        ...
                        "api_response": {"note": "CodeCraft fields missing; queued for processing."},
                        "provider": "codecraft",
                    }
                )
                continue
                ...
                results.append(
                    {
                        ...
                        "api_response": {"note": "CodeCraft package not found; queued for manual processing."},
                        "provider": "codecraft",
                    }
                )
                continue
    ...
    if order_jobs and not defer_provider_processing:
        try:
            ...
            jlog("public_checkout_bg_spawn_error", order_id=primary_order_id, error=str(e))
